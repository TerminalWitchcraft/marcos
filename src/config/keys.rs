...
impl KeySequence {
    ...
    fn emit_sequence(self) -> (bool,Vec<Event>) {
        ...
        match self {
            KeySequence{takes_count:_,
                modifier:Modifier::Alt,
                key:c} => {if !c.is_empty() {seq.push(Event::AltChar(c[0]))} },
            KeySequence{takes_count:_,
                modifier:Modifier::Ctrl,
                key:c} => {if !c.is_empty() {seq.push(Event::CtrlChar(c[0]))} },
            KeySequence{takes_count:_,
                modifier:Modifier::Shift,
                key:c} => {if !c.is_empty() {seq.push(Event::Char(c[0].to_ascii_uppercase()))} },
            ...
            KeySequence{takes_count:_,
                ...
                key:c} => {for i in c {seq.push(Event::Char(i))} },
                    _ => {},
        }
