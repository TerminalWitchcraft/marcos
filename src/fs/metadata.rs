...
impl ToString for Permissions {
    fn to_string(&self) -> String {
        ...
        if self.user_read {repr.push('r')} else {repr.push('-')}
        if self.user_write {repr.push('w')} else {repr.push('-')}
        if self.user_execute {repr.push('x')} else {repr.push('-')}

        if self.group_read {repr.push('r')} else {repr.push('-')}
        if self.group_write {repr.push('w')} else {repr.push('-')}
        if self.group_execute {repr.push('x')} else {repr.push('-')}

        if self.other_read {repr.push('r')} else {repr.push('-')}
        if self.other_write {repr.push('w')} else {repr.push('-')}
        if self.other_execute {repr.push('x')} else {repr.push('-')}
        repr
    }
}
